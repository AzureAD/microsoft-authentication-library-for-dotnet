...
package; otherwise `BrokerCredential.GetToken*` throws `CredentialUnavailableException` and the chain moves on.
(`TryCreateDevelopmentBrokerOptions` is used only to enable broker auth for `InteractiveBrowserCredential` when `UseDefaultBrokerAccount` is set.)
`BrokerCredential` is internal and is **not** one of the 20 public credential types.
