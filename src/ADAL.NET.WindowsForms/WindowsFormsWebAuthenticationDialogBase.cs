...
namespace Microsoft.IdentityModel.Clients.ActiveDirectory.Internal
{
    ...
    public abstract class WindowsFormsWebAuthenticationDialogBase : Form
    {
        ...
        protected virtual void WebBrowserNavigatingHandler(object sender, WebBrowserNavigatingEventArgs e)
        {
            ...
            if (!e.Cancel)
            {
                Logger.Verbose(null, string.Format("Navigating to '{0}'.", EncodingHelper.UrlDecode(e.Url.ToString())));
            }
        }
        ...
        private void WebBrowserNavigatedHandler(object sender, WebBrowserNavigatedEventArgs e)
        {
            ...
            {
                Logger.Verbose(null, string.Format("Navigated to '{0}'.", EncodingHelper.UrlDecode(e.Url.ToString())));
            }
        }
        ...
        private void StopWebBrowser()
        {
            if (!this.webBrowser.IsDisposed)
            {
                ...
                {
                    Logger.Verbose(null, string.Format("WebBrowser state: IsBusy: {0}, ReadyState: {1}, Created: {2}, Disposing: {3}, IsDisposed: {4}, IsOffline: {5}", this.webBrowser.IsBusy, this.webBrowser.ReadyState, this.webBrowser.Created, this.webBrowser.Disposing, this.webBrowser.IsDisposed, this.webBrowser.IsOffline));
                    this.webBrowser.Stop();
                    Logger.Verbose(null, string.Format("WebBrowser state (after Stop): IsBusy: {0}, ReadyState: {1}, Created: {2}, Disposing: {3}, IsDisposed: {4}, IsOffline: {5}", this.webBrowser.IsBusy, this.webBrowser.ReadyState, this.webBrowser.Created, this.webBrowser.Disposing, this.webBrowser.IsDisposed, this.webBrowser.IsOffline));
                }
            }
        }
