...
namespace Microsoft.Identity.Client
{
    ...
    internal static class MsalErrorMessage
    {
        ...
        public const string MtlsPopNotSupportedWithClientClaimsMessage = "mTLS Proof-of-Possession is not supported when the credential is configured via WithClientClaims(certificate, claimsToSign, ...). Custom client claims require a signed JWT-bearer client assertion, which is not produced in mTLS mode. To use mTLS PoP with a certificate, configure the application with .WithCertificate(certificate) without .WithClientClaims(...), or use .WithClientAssertion(...) with a signed assertion that carries the binding certificate. See https://aka.ms/msal-net-pop for details.";
