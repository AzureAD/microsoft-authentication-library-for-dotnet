...
{
    /// <summary>
    /// Uses the old IMDS credential endpoint to probe for the availability of the IMDS credential endpoint.
    /// Easier to test in a VM 
    /// This can be the new /issuecredential endpoint or the metadata endpoint.
    /// </summary>
    internal class ImdsCredentialProbeManager
    {
        ...
        public ImdsCredentialProbeManager(IServiceBundle serviceBundle)
        {
            _serviceBundle = serviceBundle ?? throw new ArgumentNullException(nameof(serviceBundle));
        ...
        public async Task<bool> ExecuteProbeAsync(CancellationToken cancellationToken = default)
        {
            ...
            IRetryPolicyFactory retryPolicyFactory = _serviceBundle.Config.RetryPolicyFactory;
            IRetryPolicy retryPolicy = retryPolicyFactory.GetRetryPolicy(RequestType.ManagedIdentityDefault);

            try
            {
                HttpResponse response = await _serviceBundle.HttpManager.SendRequestAsync(
                    request.ComputeUri(),
                    request.Headers,
                    httpContent,
                    request.Method,
