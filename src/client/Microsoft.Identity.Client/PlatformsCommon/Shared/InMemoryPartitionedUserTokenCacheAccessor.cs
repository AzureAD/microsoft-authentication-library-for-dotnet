...
namespace Microsoft.Identity.Client.PlatformsCommon.Shared
{
    ...
    internal class InMemoryPartitionedUserTokenCacheAccessor : ITokenCacheAccessor
    {
        ...
        public virtual List<MsalAccessTokenCacheItem> GetAllAccessTokens(string partitionKey = null, ILoggerAdapter requestlogger = null)
        {
            ...
            {
                List<MsalAccessTokenCacheItem> result = AccessTokenCacheDictionary.SelectMany(dict => dict.Value).Select(kv => kv.Value).ToList();
                if (result.Count > 1)
                {
                    Debugger.Launch();
                    Debugger.Break();
            ...
            else
            {
                ...
                var result = partition?.Select(kv => kv.Value)?.ToList() ?? CollectionHelpers.GetEmptyList<MsalAccessTokenCacheItem>();
                if (result.Count > 1)
                {
                    Debugger.Launch();
                    Debugger.Break();
