...
namespace Microsoft.Identity.Client.Internal.Requests.Silent
{
    internal class SilentRequest : RequestBase
    {
        ...
        internal ISilentAuthRequestStrategy BrokerStrategy;
        private readonly ILoggerAdapter _logger;
        private readonly bool _isBrokerConfigured;
        ...
        public SilentRequest(
            ...
            ISilentAuthRequestStrategy brokerStrategy = null,
            bool isBrokerConfigured = false)
            : base(serviceBundle, authenticationRequestParameters, silentParameters)
        {
            ...
            _isBrokerConfigured = isBrokerConfigured;
        }
        ...
        protected override async Task<AuthenticationResult> ExecuteAsync(CancellationToken cancellationToken)
        {
            ...
            if (_isBrokerConfigured && AuthenticationRequestParameters.PopAuthenticationConfiguration != null)
            {
                _logger.Info("[Silent Request] Attempting to use broker instead of searching local cache for Proof-of-Possession tokens. ");

                return await BrokerStrategy.ExecuteAsync(cancellationToken).ConfigureAwait(false);
            }
            ...
            catch (MsalException ex)
            {
                ...
                if (_isBrokerConfigured && ShouldTryWithBrokerError(ex.ErrorCode))
                {
                    _logger.Info("Attempting to use broker instead. ");
                    var brokerResult = await BrokerStrategy.ExecuteAsync(cancellationToken).ConfigureAwait(false);
