...
namespace Test.MSAL.NET.Unit.RequestsTests
{
    ...
    public class ClientCredentialRequestTests
    {
        ...
        public async Task ForceRefreshParameterTest()
        {
            ...
            var clientCredentialRequest = new ClientCredentialRequest(parameters, false);
            await clientCredentialRequest.PreTokenRequest();
            Assert.IsNotNull(clientCredentialRequest.AccessTokenItem,
                "forceRefresh == false -> Access Token from cache was used");

            clientCredentialRequest = new ClientCredentialRequest(parameters, true);
            await clientCredentialRequest.PreTokenRequest();
            Assert.IsNull(clientCredentialRequest.AccessTokenItem,
                "forceRefresh == true -> Access Token from cache was not used");
        }
    }
}
