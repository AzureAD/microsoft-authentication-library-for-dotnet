...
namespace Microsoft.Identity.Test.Integration.HeadlessTests
{
    ...
    public class ManagedIdentityTests
    {
        ...
        public async Task DirectMSI_MSAL_Async()
        {
            ...
            TestContext.WriteLine("got a token!");
        ...
        static async Task<string> GetManagedIdentityTokenAsync(
            string msiClientId, 
            string audience,
            CancellationToken cancellationToken)
        {
            var miApplication = ManagedIdentityApplicationBuilder
                .Create(ManagedIdentityId.WithUserAssignedClientId(msiClientId))
                .Build();

            var miResult = await miApplication
                .AcquireTokenForManagedIdentity($"{audience}/.default")
                .ExecuteAsync()
