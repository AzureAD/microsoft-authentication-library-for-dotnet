...
namespace Microsoft.Identity.Test.Unit.ManagedIdentityTests
{
    ...
    public class ImdsV2Tests : TestBase
    {
        ...
        public void TestCsrGeneration()
        {
            ...
            // Validate the CSR contents
            ValidateCsrContent(csrRequest.Pem, clientId, tenantId, cuid);
        }

        [TestMethod]
        public void TestCsrGeneration_InvalidClientId()
        {
            var cuid = new CuidInfo
            {
                Vmid = "test-vm-id-12345",
            ...
            Assert.ThrowsException<ArgumentException>(() => 
                CsrRequest.Generate(null, tenantId, cuid));

            // Test with empty client ID
            Assert.ThrowsException<ArgumentException>(() => 
                CsrRequest.Generate("", tenantId, cuid));

            // Test with whitespace client ID
            Assert.ThrowsException<ArgumentException>(() => 
                CsrRequest.Generate("   ", tenantId, cuid));
        }

        [TestMethod]
        public void TestCsrGeneration_InvalidTenantId()
        {
            var cuid = new CuidInfo
            {
                Vmid = "test-vm-id-12345",
            ...
            Assert.ThrowsException<ArgumentException>(() => 
                CsrRequest.Generate(clientId, null, cuid));

            // Test with empty tenant ID
            Assert.ThrowsException<ArgumentException>(() => 
                CsrRequest.Generate(clientId, "", cuid));

            // Test with whitespace tenant ID
            Assert.ThrowsException<ArgumentException>(() => 
                CsrRequest.Generate(clientId, "   ", cuid));
        }

        [TestMethod]
        public void TestCsrGeneration_InvalidCuid()
        {
            string clientId = "12345678-1234-1234-1234-123456789012";
            string tenantId = "87654321-4321-4321-4321-210987654321";
            ...
            // Test with null VMID
            var cuidWithNullVmid = new CuidInfo
            {
                Vmid = null,
                Vmssid = "test-vmss-id-67890"
            };
            Assert.ThrowsException<ArgumentException>(() => 
                CsrRequest.Generate(clientId, tenantId, cuidWithNullVmid));

            // Test with empty VMID
            var cuidWithEmptyVmid = new CuidInfo
            {
                Vmid = "",
                Vmssid = "test-vmss-id-67890"
            };
            Assert.ThrowsException<ArgumentException>(() => 
                CsrRequest.Generate(clientId, tenantId, cuidWithEmptyVmid));

            // Test with null VMSSID
            var cuidWithNullVmssid = new CuidInfo
            {
                Vmid = "test-vm-id-12345",
                Vmssid = null
            };
            Assert.ThrowsException<ArgumentException>(() => 
                CsrRequest.Generate(clientId, tenantId, cuidWithNullVmssid));

            // Test with empty VMSSID
            var cuidWithEmptyVmssid = new CuidInfo
            {
                Vmid = "test-vm-id-12345",
                Vmssid = ""
            };
            Assert.ThrowsException<ArgumentException>(() => 
                CsrRequest.Generate(clientId, tenantId, cuidWithEmptyVmssid));
        }

        [TestMethod]
        public void TestCsrGeneration_MalformedPem()
        {
            // Test parsing malformed PEM with invalid Base64 characters
            string malformedPem = "-----BEGIN CERTIFICATE REQUEST-----\nInvalid@#$%Base64Content!\n-----END CERTIFICATE REQUEST-----";
            
            Assert.ThrowsException<FormatException>(() => 
                ParseCsrFromPem(malformedPem));

            // Test with wrong headers
            string wrongHeaders = "-----BEGIN CERTIFICATE-----\nTUlJQzNqQ0NBY1lDQVFBd1pURT0K\n-----END CERTIFICATE-----";
            
            Assert.ThrowsException<ArgumentException>(() => 
                ParseCsrFromPem(wrongHeaders));
        }

        #region CSR Validation Helper Methods

        /// <summary>
        /// Validates the content of a CSR PEM string against expected values.
        /// </summary>
        private void ValidateCsrContent(string pemCsr, string expectedClientId, string expectedTenantId, CuidInfo expectedCuid)
        {
        ...
        /// </summary>
        private byte[] ParseCsrFromPem(string pemCsr)
        {
        ...
        /// </summary>
        private CsrInfo ParsePkcs10Structure(byte[] derBytes)
        {
            int offset = 0;
        ...
        /// </summary>
        private byte[] ParseAsn1Tag(byte[] data, ref int offset, byte expectedTag)
        {
        ...
        /// </summary>
        private int ParseAsn1Length(byte[] data, ref int offset)
        {
            if (offset >= data.Length)
        ...
        /// </summary>
        private void ValidateSubjectName(byte[] subjectBytes, string expectedClientId, string expectedTenantId)
        {
        ...
        /// </summary>
        private void ValidateCuidAttribute(byte[] attributesBytes, CuidInfo expectedCuid)
        {
        ...
        /// </summary>
        private void ValidateSignatureAlgorithm(byte[] signatureAlgBytes)
        {
        ...
        /// </summary>
        private bool IsOid(byte[] oidBytes, int[] expectedOid)
        {
        ...
        /// </summary>
        private byte[] EncodeOidComponent(int value)
        {
            if (value == 0)
