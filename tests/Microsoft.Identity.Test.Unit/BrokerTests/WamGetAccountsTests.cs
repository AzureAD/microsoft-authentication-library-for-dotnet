...
namespace Microsoft.Identity.Test.Unit.BrokerTests
{
    ...
    public class WamGetAccountsTests : TestBase
    {
        ...
        public async Task WAM_AccountIdWriteback_Async()
        {
            ...
            using (var httpManager = new MockHttpManager())
            {
                ...
                mockRuntime.AcquireTokenInteractiveAsync(null, null).ReturnsForAnyArgs(Task.FromResult(msalTokenResponse));
                ...
                pca2.ServiceBundle.Config.RuntimeBrokerCreatorFunc = (_, _, _) => mockRuntime;
        ...
        public async Task WAM_AccountIds_GetMerged_Async()
        {
            ...
            using (var httpManager = new MockHttpManager())
            {
                ...
                pca1.ServiceBundle.Config.RuntimeBrokerCreatorFunc = (_, _, _) => mockRuntime;
                pca2.ServiceBundle.Config.RuntimeBrokerCreatorFunc = (_, _, _) => mockRuntime;

                // Act 
                mockRuntime.AcquireTokenInteractiveAsync(null, null).ReturnsForAnyArgs(Task.FromResult(msalTokenResponse1));
                await pca1.AcquireTokenInteractive(TestConstants.s_scope).ExecuteAsync().ConfigureAwait(false);

                // this should override wam1 id
                mockRuntime.AcquireTokenInteractiveAsync(null, null).ReturnsForAnyArgs(Task.FromResult(msalTokenResponse2));
                await pca1.AcquireTokenInteractive(TestConstants.s_scope).ExecuteAsync().ConfigureAwait(false);

                mockRuntime.AcquireTokenInteractiveAsync(null, null).ReturnsForAnyArgs(Task.FromResult(msalTokenResponse3));
        ...
        public async Task GetAccounts_Returns_Both_WAM_And_Cache_Accounts_Async()
        {
            ...
            using (var httpManager = new MockHttpManager())
            {
                ...
                mockRuntime.AcquireTokenInteractiveAsync(null, null).ReturnsForAnyArgs(Task.FromResult(msalTokenResponse));
                mockRuntime.GetAccountsAsync(null, null, null, null, null).ReturnsForAnyArgs(
    ...
    {
        internal static PublicClientApplicationBuilder WithTestBroker(this PublicClientApplicationBuilder pcaBuilder, IMsalCppRuntime mockRuntime)
        {
            pcaBuilder.Config.RuntimeBrokerCreatorFunc = (_, _, _) => mockRuntime;
