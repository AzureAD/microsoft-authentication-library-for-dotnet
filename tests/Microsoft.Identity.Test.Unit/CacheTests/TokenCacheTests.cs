...
namespace Microsoft.Identity.Test.Unit.CacheTests
{
    ...
    public class TokenCacheTests : TestBase
    {
        ...
        public void FindAccessToken_ScopeCaseInsensitive()
        {
            ...
            Assert.IsNotNull(item);
            CollectionAssert.Contains(item.ScopeSet.ToList(), scopeInCache);
        }
        ...
        public async Task ValidateTokenCacheContentsAreLogged_TestAsync()
        {
            ...
            Assert.Contains(logs, "Total number of access tokens in the cache: 12");
            Assert.Contains(logs, "Total number of refresh tokens in the cache: 12");
            Assert.Contains(logs, "First 10 access token cache keys:");
            Assert.Contains(logs, "First 10 refresh token cache keys:");

            var accessTokens = cache.Accessor.GetAllAccessTokens().ToList();
            var refreshTokens = cache.Accessor.GetAllRefreshTokens().ToList();
            for (int i = 0; i < 10; i++)
            {
                StringAssert.Contains(logs, accessTokens[i].ToLogString());
                StringAssert.Contains(logs, refreshTokens[i].ToLogString());
            }
        }
