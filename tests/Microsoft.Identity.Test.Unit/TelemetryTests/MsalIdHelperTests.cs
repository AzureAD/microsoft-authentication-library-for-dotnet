...
namespace Microsoft.Identity.Test.Unit.TelemetryTests
{
    ...
    public class MsalIdHelperTests : TestBase
    {
        ...
        {
            Assert.DoesNotContain("6.2.9200", headers.ActualRequestMessage.Headers.GetValues("x-client-os").Single());
            Assert.Contains("Windows", headers.ActualRequestMessage.Headers.GetValues("x-client-os").Single());
            Assert.AreEqual(
