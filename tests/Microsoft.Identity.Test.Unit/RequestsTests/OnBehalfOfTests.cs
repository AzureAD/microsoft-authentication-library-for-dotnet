    ...
    public class OnBehalfOfTests : TestBase
    {
        ...
        public async Task AccessTokenInCache_TestAsync()
        {
            using (var httpManager = new MockHttpManager())
            {
                ...
                Assert.HasCount(0, cca.UserTokenCacheInternal.Accessor.GetAllRefreshTokens());
            }
        }
        ...
        public async Task ClaimsChallengeErrorHandling_TestAsync()
        {
            using (var httpManager = new MockHttpManager())
            {
                ...
                StringAssert.Contains(ex.Message, MsalErrorMessage.ClaimsChallenge);
                ...
                Assert.AreEqual(TestConstants.ClaimsChallenge, ex.Claims);
                Assert.Contains(ex.Message, MsalErrorMessage.ClaimsChallenge);
            }
        }
