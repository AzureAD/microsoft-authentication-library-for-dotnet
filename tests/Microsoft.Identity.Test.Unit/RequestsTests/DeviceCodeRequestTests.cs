    ...
    public class DeviceCodeRequestTests : TestBase
    {
        ...
        public async Task VerifyAuthorizationPendingErrorDoesNotLogError_Async()
        {
            ...
            using (var harness = CreateTestHarness(logCallback: (level, message, pii) =>
            {
                ...
                {
                    Assert.Fail(
                        $"Received an error message {message} and the stack trace is {new StackTrace(true)}");
                }
            ...
            }))
            {
                ...
                Assert.HasCount(2, authPendingLogs.Where(x => x.Level == LogLevel.Info).ToList(), "authorization_pending logs should be INFO");
