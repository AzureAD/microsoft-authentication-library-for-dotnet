...
namespace Microsoft.Identity.Test.Unit.WebUITests
{
    ...
    public class DefaultOsBrowserWebUiTests
    {
        ...
        public async Task DefaultOsBrowserWebUi_HappyPath_Async()
        {
            ...
            tcpInterceptor.ListenToSingleRequestAndRespondAsync(
                TestPort,
                Arg.Any<Func<Uri, string>>(),
                CancellationToken.None)
               .Returns(Task.FromResult(new Uri(TestAuthorizationResponseUri)));
            ...
            Assert.IsTrue(string.IsNullOrEmpty(authorizationResult.Code));
        }
